...
public class T01_JdbcTest {
	public static void main(String[] args) {
		/*
		 * JDBC를 이용한 데이터베이스 처리 순서 순서 : JDBC드라이버 로딩 => 해당 DB 접속 => 질의(SQL명령을 수행한다.) => 질의
		 * 결과를 받아서 처리한다. => 종료(자원반납)
		 * 
		 * 1.JDBC드라이버 로딩(오라클 기준) => JDBC드라이버는 DB를 만든 회사에서 제공한다.
		 * class.forName("oracle.jdbc.driver.OracleDriver");
		 * 
		 * 2.접속하기 : 접속이 성공하면 Connection 객체가 생서오딘다. DriverManager.getConnection()메서드를
		 * 이용한다.
		 * 
		 * 3.질의 : Statement 객체 또는 PrepareedStatement 객체를 이용하여 SQL 문장을 실행한다.
		 * 
		 * 4.결과 : 1)SQL문의 select일 경우 => ResultSet객체가 만들어 진다. ResultSet객체에는 select한 결과가
		 * 저장된다. 2)SQL문이 insert, update, delete일 경우 => 정수값을 반환한다. (정수값은 보통 실행에 성공한 레코드
		 * 수를 말한다.)
		 * 
		 */
		// DB작업에 필요한 객체변수 선언
